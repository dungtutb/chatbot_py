...
MARKETS = {
    PHILIPPIN_MARKET: {
        ...
        "last_item_file": os.path.join(FOLDER, "last_item_phi.txt"),
        "last_item_id": "",
    },
    MALAYSIA_MARKET: {
        "marketId": "60e71da9563671002b12177f",
        "title": "Đơn hàng thị trường: Malay\n",
        "last_item_file": os.path.join(FOLDER, "last_item_malay.txt"),
        "last_item_id": "",
    },
}
...
FILE_CHAT_ID = os.path.join(FOLDER, FILE_CHAT_ID)
CHATS = {}
...
def get_statistic(day: datetime):
    ...
    payload = {
        ...
        "productIds": None,
        "productComboIds": None,
        "marketIds": None,
        "mktPlatformIds": None,
        "departmentIds": None,
        "teamIds": None,
        "mktUserIds": None,
        "shippingCompanyIds": None,
        "shippingCompanyAccountIds": None,
        "warehouseIds": None,
        "mktUserHts": [],
        "orderStatuses": [],
        ...
        "adsCardIds": [],
        "extraFilters": "{}",
        "statisticType": "general",
        "orderBy": "",
        "orderType": "desc",
        "willBeDisplayedFields": {},
    }

    url = "https://tizi.asia/core-api/statistics/@/exportMktUsers?payload={}".format(
        json.dumps(payload, separators=(",", ":"))
    )
    return get_request(url)
...
async def draw_process(message, now):
...
async def draw(message):
    ...
    await draw_process(message, now)
